        ...
        try:
            ...
            # --- [State 10] 부캐 릴레이 캐릭터 자동 교체 로직 ---
                ...
                if char_index + 1 >= len(MY_CHARACTERS):
                    ...
                    bprint(f"\n🛑 [릴레이 종료] 모든 캐릭터 완료! 잠수방지 모드로 전환합니다. (⏱️ {formatted_char_time})")
                    send_blynk_notification(f"🛑 모든 캐릭터 낚시 완료 -> 잠수방지 모드 전환 (⏱️ {formatted_char_time})")
                    
                    # [보고서 및 초기화 로직 추가] 최종 결산 보고서 발송 및 일일 통계 초기화
