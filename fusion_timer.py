                ...
                if bot_mode == 5:
                    if state == 0:
                        ...
                        for pt_data in all_candidates:
                            ...
                            # [단계 2]: 5레벨 절대 보호(Priority Shield) + 특성 2-Stage 엔진
                            time.sleep(0.15)
                            ...
                            conf_lvl5 = 0.75 # 5레벨 형태 임계값 (보수적 하향)
                            conf_trait = FUSION_CONF.get('trait.png', 0.70)

                            # 판독 영역 설정 (5레벨 탐색 범위 확장)
                            col_x1, col_x2 = lx + template_label.shape[1] - 10, lx + template_label.shape[1] + 150
                            col_y1, col_y2 = max(0, ly - 30), ly + 100
                            ...
                            # [절대 원칙 1]: 5레벨 판독 (형태 OR 색상)
                            lvl5_template_val = 0
                            if roi_col_gray.size > 0:
                                lvl5_template_val = np.max(cv2.matchTemplate(roi_col_gray, t5_g, cv2.TM_CCOEFF_NORMED))
                            
                            mint_found = False
                            if roi_col_color.size > 0:
                                hsv_roi = cv2.cvtColor(roi_col_color, cv2.COLOR_BGR2HSV)
                                # 사용자 제공 데이터(166, 88, 70)를 포함하는 초광역 민트색 범위
                                # H: 166 -> OpenCV 83 / S: 88% -> 224 / V: 70% -> 179
                                lower_mint = np.array([60, 50, 50]) # 더 낮은 채도와 명도까지 흡수
                                upper_mint = np.array([100, 255, 255])
                                mint_found = cv2.countNonZero(cv2.inRange(hsv_roi, lower_mint, upper_mint)) > 3

                            # [절대 원칙 2]: 5레벨이 확인되면 '특성 검사' 자체를 생략하고 보호 모드 진입
                            if lvl5_template_val >= conf_lvl5 or mint_found:
                                is_level_5 = True
                                bprint(f"  > 🛡️ [5레벨 보호] 형태:{lvl5_template_val:.2f}, 민트색:{mint_found} -> 판독 종료")
                            ...
                            if not is_level_5:
                                ...
                                if roi_trait_color.size > 0:
                                    for scale in [0.95, 1.0, 1.05]:
                                        w, h = int(t_trait_color.shape[1] * scale), int(t_trait_color.shape[0] * scale)
                                        if w <= roi_trait_color.shape[1] and h <= roi_trait_color.shape[0]:
                                            res_t = cv2.matchTemplate(roi_trait_color, cv2.resize(t_trait_color, (w, h)), cv2.TM_CCOEFF_NORMED)
                                            trait_val = max(trait_val, np.max(res_t))
                                    
                                    if trait_val >= conf_trait:
                                        has_trait = True
                                    
                                    # [2-Stage 조기 기각]: 1차 실패 시 25% 이상일 때만 추가 대기
                                    elif trait_val >= 0.25 or lvl5_template_val >= 0.25:
                                        time.sleep(0.15)
                                        ...
                                        # 2차 5레벨 최종 재검증
                                        roi_g2 = hover_gray_2[col_y1:col_y2, col_x1:col_x2]
                                        roi_c2 = hover_color_2[col_y1:col_y2, col_x1:col_x2]
                                        v2_5 = np.max(cv2.matchTemplate(roi_g2, t5_g, cv2.TM_CCOEFF_NORMED)) if roi_g2.size > 0 else 0
                                        m2_5 = cv2.countNonZero(cv2.inRange(cv2.cvtColor(roi_c2, cv2.COLOR_BGR2HSV), lower_mint, upper_mint)) > 3 if roi_c2.size > 0 else False
                                        
                                        if v2_5 >= conf_lvl5 or m2_5:
                                            is_level_5 = True
                                        
                                        # 2차에서도 5레벨이 아니어야만 특성 최종 확정
                                        if not is_level_5:
                                            roi_t2 = hover_color_2[trait_y1:trait_y2, trait_x1:trait_x2]
                                            if roi_t2.size > 0:
                                                for scale in [0.95, 1.0, 1.05]:
                                                    w, h = int(t_trait_color.shape[1] * scale), int(t_trait_color.shape[0] * scale)
                                                    if w <= roi_t2.shape[1] and h <= roi_t2.shape[0]:
                                                        res_t2 = cv2.matchTemplate(roi_t2, cv2.resize(t_trait_color, (w, h)), cv2.TM_CCOEFF_NORMED)
