...
def toggle_stop():
    ...
    # [핵심 수정] 밝기 조절(DDC/CI) 딜레이가 걸리기 전에 전원 스위치부터 즉시 차단(폭파)합니다!
    if bot_active and run_start_time is not None:
    ...
    # 스위치가 완전히 꺼져서 메인 로직이 정지된 후, 안전하게 화면 밝기를 복구합니다.
    bprint("\n=============================================")
...
def toggle_start():
    # [버그 수정] 존재하지 않는 enable_dimming 제거
    ...
    try:
        curr_b = sbc.get_brightness()
        # [핵심] 현재 밝기가 10 이상일 때만 원본으로 저장하여 0% 무한 갇힘 버그 차단
        if curr_b and curr_b[0] >= 10: original_brightness = curr_b[0]
        sbc.set_brightness(0)
        is_dimmed = True
        bprint("  > 🌙 [절전 모드] 모니터 밝기를 0%로 낮춥니다.")
...
def safe_find_image(img_path, conf=0.6, region=None, custom_sct=None):
    ...
    global IMAGE_CACHE, IMAGE_COLOR_CACHE, ROI_SAMPLER
    ...
    try:
        ...
        if max_val < active_conf and img_path in DYNAMIC_UI_LIST:
            ...
            # 가우시안 이진화 후에도 TM_CCOEFF_NORMED (평균 감산) 대조를 사용하여 완벽한 획 분간 수행
            res_bin = cv2.matchTemplate(s_bin, t_bin, cv2.TM_CCOEFF_NORMED)
            _, bin_max_val, _, bin_max_loc = cv2.minMaxLoc(res_bin)
            
            # 창모드 배율 왜곡 대응 (96% ~ 104% 미세 가변 대조)
            if bin_max_val < active_conf:
                test_scales = [0.98, 1.02, 0.96, 1.04]
                for sc in test_scales:
                    tw = int(t_bin.shape[1] * sc)
                    th = int(t_bin.shape[0] * sc)
                    if tw < 10 or th < 10 or tw > s_bin.shape[1] or th > s_bin.shape[0]:
                        continue
                    t_bin_res = cv2.resize(t_bin, (tw, th), interpolation=cv2.INTER_AREA if sc < 1.0 else cv2.INTER_CUBIC)
                    res_bin_sc = cv2.matchTemplate(s_bin, t_bin_res, cv2.TM_CCOEFF_NORMED)
                    _, sc_max_val, _, sc_max_loc = cv2.minMaxLoc(res_bin_sc)
                    if sc_max_val >= active_conf:
                        bin_max_val = sc_max_val
                        bin_max_loc = sc_max_loc
                        template_processed = t_bin_res
                        break
            
            if bin_max_val >= active_conf:
                max_val = bin_max_val
                max_loc = bin_max_loc
                # 만약 전체화면 스캔 상태에서 적응형 이진화로 겨우 찾아낸 상황이라면, 캐시를 갱신해줌
                if is_using_cache:
                    print(f"  > 💊 [자가 치유] '{img_path}' 캐시 수명 만료 감지 (이진화)! 전체화면 좌표로 복구 완료.")
                    cache_data['master_box'] = None
                    cache_data['samples'].clear()
                    ctx['history'].clear()
                    ctx['is_locked'] = False
        ...
        if active_conf > max_val >= (active_conf - 0.04):
            ...
            # 재검증 시에도 핵심 UI인 경우 적응형 이진화 대조 분기 가동
            if max_val_re < active_conf and img_path in DYNAMIC_UI_LIST:
            ...
            try:
                ...
                if template is not None:
                    ...
                    if max_val >= 0.80 and not is_afk_processing:
                        ...
                        found_fishing = check_ui('fishing.png', 0.65)
    ...
    while True:
        try:
            ...
            if bot_active and state not in [0, -1] and ((time.time() - state_start_time > timeout_limit) or force_watchdog_reason):
                ...
                if not safe_find_image('catch_F.png', 0.70) and not safe_find_image('specific_B.png', 0.7) and not safe_find_image('fishing.png', 0.65) and not safe_find_image('fishing_mode.png', 0.70):
                    recovered = True
                ...
                if not recovered:
                    ...
                    # (3) 낚시 UI 잔존 꼬임
                    elif safe_find_image('fishing.png', 0.65) or safe_find_image('fishing_mode.png', 0.70):
                        bprint("  > [분석 3단계] 낚시 모드 잔존. 강제 취소(ESC) 시도")
                        for _ in range(10):
                            send_cmd('E'); time.sleep(0.2); send_cmd('R'); time.sleep(1.0)
                            if not safe_find_image('fishing.png', 0.65) and not safe_find_image('fishing_mode.png', 0.70):
                                recovered = True; break
            ...
            if state == 1:
                ...
                if not found_gauge and bot_active:
                    if safe_find_image('green_range.png', 0.72, region="FULL_SCREEN"):
                        found_gauge = True
            ...
            # --- [State 3] 어종 판별 및 줄 끊기 (5중 병렬 스캔 엔진) ---
                ...
                stats['daily_hook'] += 1
                stats['total_hook'] += 1
                bprint(f"[State 3] 5중 병렬 어종 분석 시작... (입질: {stats['daily_hook']}회)")
                
                found_target = False
                found_trash = False
                target_name = ""

                # 스레드들이 공유할 결과 저장소 (프레임 단위 점수표)
                frame_results = []
                result_lock = threading.Lock()

                # [분석관 스레드 함수]
                def species_scanner(img_gray, template, species_type, name):
                    if template is None: return
                    
                    res = cv2.matchTemplate(img_gray, template, cv2.TM_CCOEFF_NORMED)
                    _, max_val, _, _ = cv2.minMaxLoc(res)
                    
                    # 컷오프를 넘든 안 넘든 일단 점수를 모두 기록
                    with result_lock:
                ...
                while time.time() - start_analysis < 2.0 and bot_active:
                    ...
                    if frame_results:
                        ...
                        best_match = frame_results[0]
                        
                        elapsed_time = time.time() - start_analysis
                        
                        # 조건 A: UI가 완전히 선명해져서 점수가 0.75를 넘으면 시간 불문 즉시 낚아챔
                        # 조건 B: UI 애니메이션이 끝날 시간(0.5초)이 지났다면, 0.65 이상인 1등을 믿음
                        if best_match["score"] >= 0.75 or (elapsed_time > 0.5 and best_match["score"] >= 0.65):
                            time_str = f"{elapsed_time:.2f}초"
                            bprint(f"  > [분석 완료 - {time_str} 소요] 1위: {best_match['name']} ({best_match['score']:.3f})")
                            
                            if best_match["type"] == "TARGET":
                                found_target = True
                                target_name = best_match["name"]
                                break
                            elif best_match["type"] == "TRASH":
                                found_trash = True
                                break
                ...
                if found_target:
                    ...
                    while bot_active:
                        ...
                        # UI 잔존 여부 확인
                        if not (safe_find_image('fishing.png', 0.75) or safe_find_image('fishing_mode.png', 0.75)):
